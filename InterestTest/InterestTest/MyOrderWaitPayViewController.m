...
@interface MyOrderWaitPayViewController ()<UITableViewDataSource,UITableViewDelegate,AddGoodsDelegate>
{
...
-(void)getDate{
    ...
    [[RootHttpHelper httpHelper] getAchieveListParamsURL:@"pad-user/get-list" andParams:params andSuccess:^(NSDictionary *successData) {
        //            //加载圈圈(显示)
...
- (void)scrollsToBottomAnimated:(BOOL)animated
{
    ...
    [table scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:_dataInfoArr.count-1]  atScrollPosition:UITableViewScrollPositionBottom animated:NO];//这里一定要设置为NO，动画可能会影响到scrollerView，导致增加数据源之后，tableView到处乱跳
}
...
-(void)createView:(NSString *)attr_name{
    [self createAttributesView:attr_name];
    //    if (editView) {
    ...
    //
    //    UIView *goodsView=[[UIView alloc]initWithFrame:CGRectMake(300, 64, SCREEN_WIDTH-600, SCREEN_HEIGHT-128 )];
    //    goodsView.backgroundColor=[UIColor whiteColor];
    //    [editView addSubview:goodsView];
    //
    //    UIButton *btn=[[UIButton alloc]initWithFrame:CGRectMake( SCREEN_WIDTH-600-20-22, 20, 22, 22)];
...
- (void)createAttributesView:(NSString *)attr_name{
    ...
    for (NSDictionary *dic in _modlesArr) {
        if ([[dic objectForKey:@"attr_str"]isEqualToString:attr_name]) {
...
//        NSMutableDictionary *params = [NSMutableDictionary dictionary];
//        [params setValue:_final_Ooder_id forKey:@"order_id"];
    NSUserDefaults *userDefaultes = [NSUserDefaults standardUserDefaults];
    NSString *pad_user_room_function_id=[userDefaultes stringForKey:@"pad_user_room_function_id"];
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params setValue:pad_user_room_function_id forKey:@"pad_user_room_id"];
