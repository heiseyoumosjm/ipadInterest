...
#import "AlLiModel.h"
@interface AllAnLiViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>
{
    MickeyAlbum2 * album;
}
...
@property (strong, nonatomic) AlLiModel *goodsFrunitureModel;
@property (assign, nonatomic) NSInteger page;

@end
//好友
static NSString *ReverseCellName = @"ILikeCollectionViewCell";
@implementation AllAnLiViewController
...
-(void)getMoreDate{
    ...
    [params setValue:_IdDateArr[_postion+1] forKey:@"building_folder_id"];
    [params setValue:[NSString stringWithFormat:@"%ld",_page] forKey:@"page"];
    [params setValue:_num forKey:@"num"];
    
    [[RootHttpHelper httpHelper] getAchieveListParamsURL:@"building/building-case" andParams:params andSuccess:^(NSDictionary *successData) {
            _postion=_postion+1;
        //            //加载圈圈(显示)
        if ([[successData objectForKey:@"api_code"] integerValue]==200) {
            ...
            for (NSMutableDictionary *dic in arr) {
                ...
                _goodsFrunitureModel=[[AlLiModel alloc]initWithDictionary:dic error:nil];
...
-(void)getDate
{
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params setValue:_cateId forKey:@"building_folder_id"];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    _goodsFrunitureModel=[[AlLiModel alloc]init];
...
//    img.image=[UIImage imageNamed:@""]
    [img sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",@"http://weixin.inhomehz.com",_goodsFrunitureModel.case_pics]] placeholderImage:[UIImage imageNamed:@"默认"]];
...
//     img.layer.shadowRadius = 30;//阴影半径，默认3
//
    ...
    styleLab.text=[NSString stringWithFormat:@"%ld %@",(long)indexPath.row+1,_goodsFrunitureModel.case_name];
    //
...
-(void)likeChange:(UIButton *)btn1{
    ...
    if ([_goodsFrunitureModel.is_collection integerValue]==0) {
        ...
        [params setValue:@"case" forKey:@"type"];
...
//    for (NSString *str in _goodsFrunitureModel.imgs) {
        [_imageArr addObject:[NSString stringWithFormat:@"%@%@",@"http://weixin.inhomehz.com",_goodsFrunitureModel.case_pics]];
//    }
    album = [[MickeyAlbum2 alloc]initWithImgUrlArr:_imageArr CurPage:0];
