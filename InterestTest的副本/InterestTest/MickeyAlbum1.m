...
-(void)setMemer
{
    isBigger = NO;
    maxScale = 3.0;
    maxSize = CGSizeMake(myScreenWidth, myScreenHeight);
    minSize = CGSizeMake(myScreenWidth/2, myScreenHeight/2);
}
-(void)setWholeScoll
{
    wholeScoll = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    wholeScoll.backgroundColor = UIColorFromRGB(0xffffff);
...
-(void)setImagesLocationAlbum
{
    ...
    for (int i = 0; i<mImgLocationArr.count; i++) {
        ...
        imgview.image = img;
//        [imgview rotate360DegreeWithImageView];
...
-(void)setImagesUrlAlbum
{
    ...
    for (int i = 0; i<mImgUrlArr.count; i++) {
        ...
        UIScrollView * singleview = [[UIScrollView alloc]initWithFrame:CGRectMake(myScreenWidth*i,0,myScreenWidth, myScreenHeight)];
        [singleview setBackgroundColor:UIColorFromRGB(0xffffff)];
        UIImageView * imgview = [[UIImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2- 1239/2/2, SCREEN_HEIGHT/2-1007/2/2, 1239/2, 1007/2)];
        imgview.contentMode = UIViewContentModeScaleAspectFit;
//        imgview.layer.shadowColor = [UIColor blackColor].CGColor;//shadowColor阴影颜色
//        imgview.layer.shadowOffset = CGSizeMake(0,30);//shadowOffset阴影偏移,x向右偏移4，y向下偏移4，默认(0, -3),这个跟shadowRadius配合使用
//        imgview.layer.shadowOpacity = 0.2;//阴影透明度，默认0
//        imgview.layer.shadowRadius = 30;//阴影半径，默认3
        
//        UIImageView *img=[[UIImageView alloc]initWithFrame:CGRectMake(singleview.frame.size.width/2-79/2, myScreenHeight-45-64, 79, 44)];
//        img.image=[UIImage imageNamed:@"img360"];
//        [singleview addSubview:img];
...
-(void)doubleTapOnScrollView:(UITapGestureRecognizer*)doubletap
{
    ...
    //    MickeyAlbumOutImgViewPointType pointType = 0;
    //
    //    if (tapY<imgY) {
    //        //上面
    //        pointType = MickeyAlbumOutImgViewPointLeftUp;
    //        if (tapX>myScreenWidth/2) {
    //            pointType = MickeyAlbumOutImgViewPointRightUp;
    //        }
    //        isOutImgView = YES;
    //    }
    ...
    //    {
    //        //下面
    //        pointType = MickeyAlbumOutImgViewPointLeftDown;
    //        if (tapX>myScreenWidth/2) {
    //            pointType = MickeyAlbumOutImgViewPointRightDown;
    //        }
    //        isOutImgView = YES;
    //    }
    //
    ...
    //
    //                switch (pointType) {
    //                    case MickeyAlbumOutImgViewPointLeftUp:
    //                    {
    //                        singleScrollView.contentOffset = CGPointMake(0, 0);
    //                    }
    //                        break;
    //                    case MickeyAlbumOutImgViewPointLeftDown:
    //                    {
    //                        singleScrollView.contentOffset = CGPointMake(0, imgview.frame.size.height - myScreenHeight);
    //                    }
    //                        break;
    //                    case MickeyAlbumOutImgViewPointRightDown:
    //                    {
    ...
    //                    }
    //                        break;
    //                    case MickeyAlbumOutImgViewPointRightUp:
    //                    {
